  ...
  useEffect(() => {
    ...
    if (isLoading && !hasError) {
      const timeoutId = setTimeout(() => {
        ...
        if (isLoading) {
          console.error("Iframe loading timeout reached. Assuming failure and displaying fallback.");
          setHasError(true);
          setIsLoading(false);
        }
  ...
  return (
    <main className="min-h-screen flex flex-col justify-center items-center bg-gray-50 p-6">
      <div className="w-full max-w-3xl space-y-6">
        ...
        {/* Iframe container */}
        <div
          className="relative w-full aspect-video border rounded overflow-hidden shadow bg-black"
        >
          <iframe
            title={game.title}
            allow="fullscreen"
